...
5. runs a high-severity production dependency audit;
6. creates a bcrypt-hashed admin credential and validated config;
7. creates an installation sentinel and restrictive permissions;
8. installs/enables a version-controlled systemd unit;
9. starts the service;
10. polls `/readyz` before reporting success.
...
The current loader distinguishes:
...
State writes use a temporary file plus atomic rename and errors propagate to readiness/shutdown rather than being silently converted to empty state.

The legacy installer `dataEncKey` is migrated to the dedicated key file before legacy encrypted data is rewritten. Upgrade tests verify users/messages/permissions survive the migration.
...
Production frontend dependencies are built and committed as self-hosted assets. CI reruns the build and requires the generated files to have no diff.
...
The updater uses an external atomic lock with stale-lock handling, creates and verifies a backup, stages the release, runs `npm ci`, validates/builds the release, swaps code without deleting persistent data, restarts and polls readiness. On post-swap failure it restores the previous code and verifies the rollback.
...
Runtime `data/`, uploads, backups, logs, keys and locks are excluded from Git.
